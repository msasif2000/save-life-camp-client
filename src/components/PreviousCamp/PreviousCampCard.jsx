...
const PreviousCampCard = ({ camp }) => {
    ...
    return (
        <div className="flex flex-col shadow-2xl shadow-third transition-all hover:-translate-y-1 hover:scale-105  duration-300 rounded-lg bg-second">
            ...
            <div >
                ...
                <div data-aos="fade-up" data-aos-duration="1000" className=" px-4 py-2 flex justify-end">
                    <Link to={`/detailsCamp/${_id}`}><button className="btn border-third bg-grn hover:bg-prime hover:text-black text-white"><BsFillArrowUpRightCircleFill className="text-2xl" /> Details</button></Link>
                </div>
            </div>
        </div>
    );
